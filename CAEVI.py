...
from utils.dict_utils import caevi_rh_dict
from utils.dict_utils import caevi_val_servicios
...
st.subheader('Centro de atención especializada a la violencia interpersonal')
...
for key, value in list(caevi_rh_dict.items()):
...
if (len(data["porcentaje_subsidiado"]) <= 2 or len(data["porcentaje_subsidiado"]) >= 4):
    col4.warning("Inserte un porcentaje válido")

data["porcentaje_capacitado"] = col4.text_input("¿Cuál es el porcentaje del personal del centro capacitado en el nuevo modelo CAEVI?" "%")

if (len(data["porcentaje_subsidiado"]) <= 2 or len(data["porcentaje_subsidiado"]) >= 4):
    col4.warning("Inserte un porcentaje válido")

data["manual_proc"] = col4.selectbox("¿El centro cuenta con manual de procedimiento?", ["No", "Sí", "No Aplica"])
...
for key, value in list(caevi_val_servicios.items())[:7]:
...
if sub.button('Enviar'):
    ...
    if (emp == True):
        st.warning('**Por favor verifique que se ingresaron todos los campos**')

    else:
        caevi_collection.insert_one(data)
